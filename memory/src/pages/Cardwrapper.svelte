<script>
    import { onMount } from "svelte";
    import {
        allBodies,
        selectedBodies,
        sortKey,
        sortDirection,
    } from "../store.js";
    import Card from "../components/Card.svelte";

    let displayedBodies = [];

    const unsubscribe = selectedBodies.subscribe((value) => {
        displayedBodies = value.slice(); // Make a copy of selected bodies
        handleSort(sortKey); // Sort initially based on the current sort key
    });

    function sortPlanets(bodies, key) {
        return bodies.sort((a, b) => {
            if (sortDirection === "asc") {
                return a[key] - b[key];
            } else {
                return b[key] - a[key];
            }
        });
    }

    function handleSort(key) {
        sortKey.set(key);
        sortDirection.set("asc"); // Default sorting direction
        displayedBodies = sortPlanets(displayedBodies, key);
    }

    onMount(() => {
        handleSort(sortKey); // Ensure initial sorting
        return () => {
            unsubscribe();
        };
    });
</script>

<div>
    <button on:click={() => handleSort("semimajorAxis")}>Semimajor Axis</button>
    <button on:click={() => handleSort("meanRadius")}>Mean Radius</button>
    <button on:click={() => handleSort("gravity")}>Gravity</button>
    <button on:click={() => handleSort("sideralOrbit")}>Sideral Orbit</button>
    <button on:click={() => handleSort("density")}>Density</button>
</div>

<div id="card-container">
    {#each displayedBodies as body}
        <Card {body} {sortKey} />
    {/each}
</div>

<style>
    button {
        width: 150px;
        padding: 5px 10px;
        background-color: #1b263d;
        color: white;
        border-radius: 5px;
        cursor: pointer;
        font-size: 1em;
        border: 1px solid #ddd;
        text-align: center;
        margin: 20px;
    }
    #card-container {
        display: grid;
        grid-template-columns: repeat(2, 1fr);
        gap: 1em;
        color: #222;
        cursor: pointer;
        transition: transform 0.3s ease-in-out;
        margin-top: 20px;
    }

    @media screen and (max-width: 768px) {
        #card-container {
            display: grid;
            place-items: center;
            grid-template-columns: repeat(2, 1fr);
            gap: 0.5em;
            margin-top: 20px;
        }
    }
</style>
